...
export default function Header() {
  ...
  return (
    ...
    <header className="fixed top-0 left-0 w-full z-50 py-3 px-4 md:px-6">
      
    
      <div className="max-w-7xl mx-auto bg-white/70 backdrop-blur-lg rounded-full border border-slate-100 shadow-lg shadow-slate-900/5">
        
        <nav className="h-16 flex justify-between items-center px-6 text-slate-700">
          ...
          {!isAuthenticated ? (
            <div className="flex gap-4 items-center">
              ...
              <Link
                to="/register"
                className="bg-gradient-to-r from-orange-500 to-amber-500 text-white px-5 py-2.5 rounded-full font-bold text-sm shadow-md shadow-orange-500/10 hover:shadow-lg hover:shadow-orange-500/20 active:scale-[0.98] transition-all"
              >
                Register
              </Link>
            </div>
            ...
            <div className="flex items-center gap-4 relative" ref={dropdownRef}>
              ...
              <button
                ...
                className="flex items-center gap-2.5 hover:bg-slate-50/50 px-3 py-1.5 rounded-full transition-all border border-slate-100/50 shadow-sm"
              >
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 top-[calc(100%+0.75rem)] w-56 bg-white/95 backdrop-blur-md border border-slate-100 rounded-2xl shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-[calc(100%-0.5rem)] left-4 right-4 bg-white/95 backdrop-blur-lg border border-slate-100 shadow-xl rounded-2xl py-4 px-6 flex flex-col gap-4 font-semibold text-base animate-in slide-in-from-top duration-200 z-40">
          <Link 
            to="/" 
            onClick={() => setIsMobileMenuOpen(false)}
